...
void HLTInfo::analyze(const edm::Handle<edm::TriggerResults>                 & hltresults,
  ...
  if (hltresults.isValid()) {
	...
	if(addThisBranch){
	  ...
	  HltTree->Branch(trigName+"_trigPt",trigObject+itdum);
          trigflag[itdum] = 0;
	  trigPrescl[itdum] = 0;
          //trigPt[itdum] = 0;
	  ++itdum;
    ...
    for (int ijij = 0; ijij<ntrigs; ijij++){ trigObject[ijij].clear();}
    ...
    for (int itrig = 0; itrig != ntrigs; ++itrig){
      ...
      for(unsigned int im=0; im<hltConfig_.moduleLabels(trigName).size(); im++){
          ...
          //if(filterIndex<triggerEventHandle_->sizeFilters())
              //cout<<"filterIndex: "<<filterIndex<<" triggerEventHandle_->sizeFilters(): "<<triggerEventHandle_->sizeFilters()<<endl;
          if (filterIndex<triggerEventHandle_->sizeFilters()) {
              ...
              for (trigger::size_type i=0; i!=n; ++i) {
                  ...
                  //cout <<trigName<<" "<<moduleLabel<<" "<<moduleType<< "   " << i << " " << VIDS[i] << "/" << KEYS[i] << ": " << TO.id() << " " << TO.pt() << " " << TO.eta() << " " << TO.phi() << " " << TO.mass() << endl;
